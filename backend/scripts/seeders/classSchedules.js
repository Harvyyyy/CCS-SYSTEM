...
const seedClassSchedulesCore = async ({ reset = false, userMap } = {}) => {
  ...
  const [sections, courses, facultyProfiles] = await Promise.all([
    Section.find({ schoolYearSemester: currentSchoolYear._id }),
  ...
  for (let i = 0; i < sections.length; i += 1) {
    ...
    const targetCourses = courses.filter((course) => course.year === sectionYear);
    const usableCourses = targetCourses.length > 0 ? targetCourses : courses;
    const desiredCount = Math.min(6, usableCourses.length);
    ...
    for (let j = 0; j < desiredCount; j += 1) {
      ...
      const scheduleType = j % 3 === 0 ? "Laboratory" : "Lecture";
      const roomName = ROOMS[(i + j + (scheduleType === "Laboratory" ? 6 : 0)) % ROOMS.length];
